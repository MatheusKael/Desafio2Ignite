...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  const verifie = user.todos.length + 1;

  if (verifie > 10 && user.pro === false) {
    return response
      .status(403)
      .json({ error: "O seu plano gratis atingiu o limite" });
  }
...
function checksTodoExists(request, response, next) {
  ...
  if (!validation) {
    return response.status(400).json({ error: "Id nao é um UUID" });
  }
